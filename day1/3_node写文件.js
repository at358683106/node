...
let res = [];
testData.cityList.forEach((item) => {
  item.cities.forEach((item2) =>
    res.push({ name: item2.name, lat: item2.latitude, lng: item2.longitude })
  );
});

fs.writeFile(
  path.resolve(__dirname, "file/test2.json"),
  JSON.stringify({ citys: res }),
  (error) => {
    if (error) return console.log("文件写入失败");
    console.log("文件写入成功");
  }
